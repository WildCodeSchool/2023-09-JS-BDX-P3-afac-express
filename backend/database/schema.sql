...
CREATE TABLE if not exists `artist` (
    `id` int NOT NULL AUTO_INCREMENT, `name` varchar(255) NOT NULL, `description` text, `image` varchar(500) DEFAULT NULL, PRIMARY KEY (`id`)
...
CREATE TABLE if not exists `artwork` (
    `id` int NOT NULL AUTO_INCREMENT, `title` varchar(200) NOT NULL, `image` varchar(500) DEFAULT NULL, `dimension` varchar(100) DEFAULT NULL, `creation_place` varchar(255) DEFAULT NULL, `artist_id` int DEFAULT NULL, PRIMARY KEY (`id`)
...
INSERT INTO
    `artwork`
VALUES (
        1, 'Tirem nos tudo mas deixem nos a música...', 'https://i.postimg.cc/XJ1h3qYd/1-Tirem-Nos-Tudo-Mas-Deixem-Nosamusica-Quandoeu-Nasci-Era-Meio-Diaeo-Sol-Brillava-regressaremos-Sombrias-Corp.jpg', '', '', 1
    ),
    (
        2, 'Sementes e imagens', 'https://i.postimg.cc/qqX1q7k1/2-Sementes-EImagens.jpg', '', '', 1
    ),
    (
        3, 'Quando eu nasci era...', 'https://i.postimg.cc/g0VXfL4t/3-Quando-Eu-Nasci-Era.jpg', '', '', 1
    ),
    (
        4, '... era meio dia', 'https://i.postimg.cc/x8zS4M3c/4-Era-Meio-Dia.jpg', '', '', 1
    ),
    (
        5, '... e o sol', 'https://i.postimg.cc/VLz8RrbL/5-EOSol.jpg', '', '', 1
    ),
    (
        6, 'Brède chouchou', 'https://i.postimg.cc/HnZBXhyM/6-Brede-Chouchou.jpg', '1200 x 1600', 'Maurice', 2
    ),
    (
        7, 'Brède rave', 'https://i.postimg.cc/x8zc3kkP/7-Brede-Rave.jpg', '1200 x 1600', 'Maurice', 2
    ),
    (
        8, 'Brède tompouce canvas', 'https://i.postimg.cc/bN0xtv7h/8-Brede-Tompouce-Canvas.jpg', '1158 X 1600', 'Maurice', 2
    ),
    (
        9, 'Brède tompouce papier', 'https://i.postimg.cc/3w2YKPyn/9-Brede-Tompouce-Papier.jpg', '1158 X 1600', 'Maurice', 2
    ),
    (
        10, 'Brède songe', 'https://i.postimg.cc/HkZBrxML/10-Brede-Songe.jpg', '4032 x 2172', 'Maurice', 2
    ),
    (
        11, 'Analakely, Antananarivo', 'https://i.postimg.cc/DZqN4hFY/11-Analakely-Antananarivo.jpg', '4613 x 4613', 'Madagascar', 3
    ),
    (
        12, 'Ancienne banque de Madagascar et des Comores, Toamasina', 'https://i.postimg.cc/tRkrK2gP/12-Ancienne-Banque-De-Madagascar-Et-Des-Comores-Toamasina.jpg', '', 'Madagascar', 3
    ),
    (
        13, 'Cathédrale , Antsirabe', 'https://i.postimg.cc/HxjBg7km/13-Cath-drale-Antsirabe.jpg', '4771 x 4771', 'Madagascar', 3
    ),
    (
        14, 'Cathédrale Andohalo, Antananarivo', 'https://i.postimg.cc/hvBz0HwY/14-Cathedrale-Andohalo-Antananarivo.jpg', '', 'Madagascar', 3
    ),
    (
        15, 'Gare d\'Antananarivo', 'https://i.postimg.cc/jj6r7mH2/15-Gare-DAntananarivo.jpg', '6906 x 6906', 'Madagascar', 3
    ),
    (
        16, 'Il ne reste plus que...', 'https://i.postimg.cc/HxRDDmHS/16-Il-Ne-Reste-Plus-Que.jpg', '', 'Réunion', 4
    ),
    (
        17, '... réalité fantomatique', 'https://i.postimg.cc/4xN85MRn/17-Realite-Fantomatique.jpg', '', 'Réunion', 4
    ),
    (
        18, 'Semis du riz. Métamorphosés... sublimés', 'https://i.postimg.cc/HLMb44nV/18-Semis-Du-Riz-Metamorphoses-sublimes.jpg', '', 'Réunion', 4
    ),
    (
        19, '... une vertue intrinsèque l\'est au corps', 'https://i.postimg.cc/HxH8wqFh/19-Une-Vertue-Intrinseque-lest-Au-Corps.jpg', '', 'Réunion', 4
    ),
    (
        20, 'Loading drums of alcohol... rêves', 'https://i.postimg.cc/P5sXvmg6/20-Loading-Drums-Of-Alcohol-Reves.jpg', '', 'Réunion', 4
    )
On DUPLICATE KEY UPDATE
    `id` = VALUES(`id`),
    `title` = VALUES(`title`),
...
CREATE TABLE if not exists `artwork_users` (
    `users_id` int NOT NULL, `artwork_id` int NOT NULL, `artist_id` int NOT NULL, `artist_name` varchar(255) NOT NULL, `artwork_title` varchar(200) NOT NULL, `artwork_image` varchar(500) NOT NULL, KEY `fk_artwork_users_users` (`users_id`), KEY `fk_artwork_users_artwork` (`artwork_id`), KEY `fk_artwork_users_artist` (`artist_id`), CONSTRAINT `fk_artwork_users_users` FOREIGN KEY (`users_id`) REFERENCES `users` (`id`), CONSTRAINT `fk_artwork_users_artist` FOREIGN KEY (`artist_id`) REFERENCES `artist` (`id`), CONSTRAINT `fk_artwork_users_artwork` FOREIGN KEY (`artwork_id`) REFERENCES `artwork` (`id`)
...
CREATE TABLE if not exists `users` (
    `id` int NOT NULL AUTO_INCREMENT, `firstname` varchar(255) NOT NULL, `lastname` varchar(255) NOT NULL, `email` varchar(255) NOT NULL, `password` varchar(255) NOT NULL, `is_admin` tinyint(1) DEFAULT '0', `secret_question` varchar(255) DEFAULT NULL, `secret_answer` varchar(255) DEFAULT NULL, PRIMARY KEY (`id`), UNIQUE KEY `email` (`email`)
